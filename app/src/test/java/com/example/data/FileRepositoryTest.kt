...
class FileRepositoryTest {
    ...
    class FakeFileDao : FileDao {
        ...
        override suspend fun updateFile(file: FileItemEntity) { updatedFile = file; updateFileCallCount++ }
        override suspend fun getFilteredFilesPaged(category: String?, query: String, limit: Int, offset: Int): List<FileItemEntity> { getFilteredFilesPagedCalled = true; lastCategory = category; lastQuery = query; lastLimit = limit; lastOffset = offset; return pagedReturnList }
    ...
    @Before fun setUp() { context = RuntimeEnvironment.getApplication(); fakeDao = FakeFileDao(); repository = FileRepository(context, fakeDao) }

    @Test fun testRenameFile_success_updatesDatabase() = runBlocking {
        val tempFile = File.createTempFile("test_file_", ".txt"); tempFile.writeText("sample content")
        val originalFile = FileItemEntity(id = 1L, name = tempFile.name, path = tempFile.absolutePath, category = "DOCUMENTS", sizeBytes = tempFile.length())
        val newName = "renamed_file.txt"
        try {
            val updatedFile = repository.renameFile(originalFile, newName)
            assertEquals(newName, updatedFile.name); assertTrue(updatedFile.path.endsWith(newName)); assertEquals(1, fakeDao.updateFileCallCount); assertEquals(1L, fakeDao.updatedFile?.id); assertEquals(newName, fakeDao.updatedFile?.name); assertEquals(updatedFile.path, fakeDao.updatedFile?.path)
        } finally { if (tempFile.exists()) tempFile.delete(); val renamedFile = File(tempFile.parentFile, newName); if (renamedFile.exists()) renamedFile.delete() }
    }

    @Test fun testRenameFile_failure_throwsExceptionAndDoesNotUpdateDatabase() = runBlocking {
        val originalFile = FileItemEntity(id = 2L, name = "non_existent_file.txt", path = "/non_existent_directory_vvf/non_existent_file.txt", category = "DOCUMENTS", sizeBytes = 100L)
        try { repository.renameFile(originalFile, "should_fail.txt"); fail("Expected exception for non-existent file rename") } catch (e: Exception) { assertTrue(e is FileNotFoundException || e is IOException) }
        assertEquals(0, fakeDao.updateFileCallCount); assertNull(fakeDao.updatedFile)
    }

    @Test fun testAddTagToFile_emptyTags() = runBlocking {
        val fileWithNoTags = FileItemEntity(id = 3L, name = "photo.jpg", path = "/storage/photo.jpg", category = "IMAGES", sizeBytes = 500L, tags = "")
        val result = repository.addTagToFile(fileWithNoTags, "nature")
        assertEquals("nature", result.tags); assertEquals(1, fakeDao.updateFileCallCount); assertEquals(3L, fakeDao.updatedFile?.id); assertEquals("nature", fakeDao.updatedFile?.tags)
    }

    @Test fun testAddTagToFile_existingTagsPreserved() = runBlocking {
        val fileWithTags = FileItemEntity(id = 4L, name = "photo.jpg", path = "/storage/photo.jpg", category = "IMAGES", sizeBytes = 500L, tags = "family, vacation")
        val result = repository.addTagToFile(fileWithTags, "2026")
        assertEquals("family, vacation, 2026", result.tags); assertEquals(1, fakeDao.updateFileCallCount); assertEquals(4L, fakeDao.updatedFile?.id); assertEquals("family, vacation, 2026", fakeDao.updatedFile?.tags)
    }

    @Test fun testGetFilteredFilesPaged_delegatesToDaoWithExactParams() = runBlocking {
        val expectedList = listOf(FileItemEntity(id = 5L, name = "report.pdf", path = "/report.pdf", category = "DOCUMENTS", sizeBytes = 1024L))
        fakeDao.pagedReturnList = expectedList
        val result = repository.getFilteredFilesPaged("DOCUMENTS", "report", 10, 0)
        assertEquals(expectedList, result); assertTrue(fakeDao.getFilteredFilesPagedCalled); assertEquals("DOCUMENTS", fakeDao.lastCategory); assertEquals("report", fakeDao.lastQuery); assertEquals(10, fakeDao.lastLimit); assertEquals(0, fakeDao.lastOffset)
    }
}
