...
class DuplicateManagerTest {
    ...
    class FakeFileDao : FileDao {
        ...
        override suspend fun getFileById(id: Long): FileItemEntity? = filesMap[id]
        override suspend fun findInRecycleBinByHash(hash: String): FileItemEntity? = movedFilesList.find { it.md5Hash == hash && it.isRecycleBin }
        override suspend fun moveFilesToRecycleBinAtomic(files: List<FileItemEntity>) { moveFilesToRecycleBinAtomicCallCount++; movedFilesList.addAll(files); files.forEach { filesMap[it.id] = it } }
        override suspend fun updateFiles(files: List<FileItemEntity>) { movedFilesList.addAll(files) }
        ...
        override suspend fun insertFileDirect(file: FileItemEntity): Long { filesMap[file.id] = file; return file.id }
        override suspend fun getAllOrdinaryFilesDirect(): List<FileItemEntity> = filesMap.values.filter { !it.isVault && !it.isRecycleBin }
        override suspend fun deleteFilesByIds(ids: List<Long>) { ids.forEach { filesMap.remove(it) } }
    ...
    fun same_file_id_in_multiple_duplicate_groups_results_in_single_recycle_bin_operation() = runBlocking {
        ...
        val testFile = FileItemEntity(id = 101L, name = "test_duplicate.png", path = "/storage/emulated/0/Pictures/test_duplicate.png", category = "IMAGES", sizeBytes = 2048L, md5Hash = "content_hash_12345")
        fakeDao.filesMap[testFile.id] = testFile
        val duplicateManager = DuplicateManager(fakeDao)
        duplicateManager.cleanSelectedDuplicates(setOf(101L, 101L))
    ...
    fun content_hash_idempotency_check_prevents_reprocessing_already_recycled_hash() = runBlocking {
        val fakeDao = FakeFileDao()
        val recycledFile = FileItemEntity(id = 101L, name = "test_duplicate.png", path = "/storage/emulated/0/Pictures/test_duplicate.png", category = "IMAGES", sizeBytes = 2048L, md5Hash = "content_hash_12345", isRecycleBin = true)
        val activeFile = FileItemEntity(id = 102L, name = "test_duplicate_copy.png", path = "/storage/emulated/0/Pictures/test_duplicate_copy.png", category = "IMAGES", sizeBytes = 2048L, md5Hash = "content_hash_12345", isRecycleBin = false)
        fakeDao.filesMap[recycledFile.id] = recycledFile
        fakeDao.filesMap[activeFile.id] = activeFile
        fakeDao.movedFilesList.add(recycledFile)
        val duplicateManager = DuplicateManager(fakeDao)
